...
public class FileManager {
    ...
    /**
     * Метод, который использовался для тестов на раннем этапе написания программы. Был оставлен, т.к. возможно понадобится в будущем
     * @param collection
     * @param filename
     * @throws IOException
     */
    public static void saveCollectionForTests(PriorityQueue<Movie> collection, String filename) throws IOException {
        FileOutputStream fos = new FileOutputStream(path + "/" + filename);
        String data = Parser.parseToJSONWithoutDate(collection);
        byte[] buffer = data.getBytes();
        fos.write(buffer);
    }
    ...
    private static class Parser {
        ...
        /**
         * Метод конвертирует коллекцию в текстовый формат JSON (игнорирует поле creationDate)
         ...
         */
        private static String parseToJSONWithoutDate(PriorityQueue<Movie> collection) {
            String data = "";
            data += "{\n\tMovieCollection: [";
...
//загрузка из любого файла полностью через перменную окружения
// remove_by_id 1 молча ничего не делает на пустой коллекции
//update на пустой коллекции